...
class TestPhotoVerification(ModuleStoreTestCase):
    ...
    def test_display(self):
        user = UserFactory.create()
        attempt = SoftwareSecurePhotoVerification(user=user, status="denied")
        attempt.save()

        # We expect the verification to be displayed by default
        self.assertEquals(SoftwareSecurePhotoVerification.display_status(user), True)

        # Turn it off
        SoftwareSecurePhotoVerification.display_off(user.id)
        self.assertEquals(SoftwareSecurePhotoVerification.display_status(user), False)
